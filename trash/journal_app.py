...
class JournalApp:
    ...
    def create_menu(self):
        ...
        file_menu.add_command(label="Создать новый файл", command=self.create_new_file)
    ...
    def create_file_frame(self, parent):
        ...
        ttk.Button(btn_frame, text="Выбрать файл", command=self.open_file).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="Создать новый", command=self.create_new_file).pack(side=tk.LEFT, padx=2)
    ...
    def create_new_file(self):
        ...
        if filename:
            try:
                ...
                # Создаем базовые листы
                months = ["09_Сентябрь", "10_Октябрь", "11_Ноябрь", "12_Декабрь", 
                         "01_Январь", "02_Февраль", "03_Март", "04_Апрель", "05_Май"]
                
                # Удаляем лист по умолчанию
                wb.remove(wb.active)
                
                # Создаем листы для месяцев
                for month in months:
                ...
                # Создаем семестровые листы
                autumn = wb.create_sheet("осень")
                spring = wb.create_sheet("весна")
                
                # Базовые заголовки для семестровых листов
                for sheet in [autumn, spring]:
                    sheet['D4'] = "Дисциплина"
                    sheet['E4'] = "Группа"
                    sheet['F4'] = "Вид нагрузки"
                
                wb.save(filename)
                self.load_workbook(filename)
                messagebox.showinfo("Успех", f"Новый файл создан: {filename}")
...
1. ВЫБОР ФАЙЛА
   ...
   - Или "Создать новый" для создания шаблона
...
Разработчики: Корнеева Дарья, Курдюмова Виолетта, Гркикян Мисак.
        """
    ...
    def show_info_window(self, title, text):
        ...
        ttk.Button(frame, text="Закрыть", command=window.destroy).grid(row=1, column=0, pady=10)
    ...
    # Остальные методы остаются практически без изменений, только добавляем проверки на наличие файла
    ...
    # Все остальные методы (create_period_frame, create_dates_frame, create_input_frame, 
    # create_gif_frame, create_view_frame, и все функциональные методы) остаются такими же,
    # как в предыдущей версии, но добавляем проверки на self.wb в критических местах
    ...
    def create_dates_frame(self, parent):
        ...
        self.single_date = DateEntry(frame, width=12, background='darkblue', foreground='white',
                                   borderwidth=2, date_pattern='dd.mm.yyyy', locale='ru_RU')
        self.single_date.grid(row=0, column=1, sticky=tk.W, padx=(0, 15))
        
        ttk.Button(frame, text="Добавить дату", command=self.add_single_date).grid(row=0, column=2, sticky=tk.W, padx=5)
        ...
        ttk.Button(frame, text="Удалить дату", command=self.remove_selected_date).grid(row=1, column=2, sticky=tk.W, padx=5, pady=(10, 0))
        
        # Список выбранных дат
        ttk.Label(frame, text="Выбранные даты:").grid(row=2, column=0, sticky=tk.W, pady=(10, 5))
        
        self.dates_listbox = tk.Listbox(frame, width=40, height=8)
    ...
    def create_input_frame(self, parent):
        ...
        for i, (label, field, field_type) in enumerate(fields):
            ...
            if field_type == "combobox":
                ...
                self.entries[field] = ttk.Entry(frame, width=28)
    ...
    def create_view_frame(self, parent):
        ...
        ttk.Button(btn_frame, text="Обновить данные", 
                  command=self.show_data).pack(side=tk.LEFT, padx=2)
    ...
    # Все функциональные методы (on_tree_select, select_all_entries, deselect_all_entries, 
    # delete_selected_entries, delete_row, date_to_datetime, determine_week_type, find_sheet_for_month,
    # generate_dates_by_period, add_single_date, remove_selected_date, update_dates_display, clear_dates,
    # update_dates_info, show_data, add_entries, fill_season_sheets, fill_season_sheet,
    # add_entry_to_sheet, shift_rows_down, insert_entry_sorted, fill_row_data) 
