...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                               ...
                               QDateEdit, QRadioButton, QButtonGroup, QCheckBox,
...
class JournalApp(QMainWindow):
        ...
        if sheet_name and sheet_name in self.wb.sheetnames:
            ...
            try:
                ...
                if sheet_name.lower() in ['осень', 'весна']:
                    self.show_season_data(sheet, sheet_name)
                else:
                    # Для месячных листов используем стандартный формат
                    self.show_monthly_data(sheet)
    ...
    def show_monthly_data(self, sheet):
        """Показывает данные из месячного листа"""
        row = self.START_ROW
        data_rows = []
    ...
    def show_season_data(self, sheet, sheet_name):
        """Показывает данные из семестрового листа (осень/весна)"""
        # Для семестровых листов данные находятся в колонках D, E, F, G, H, I, J
        row = 5
        data_rows = []
    ...
    def delete_selected_entries(self):
        ...
        try:
            ...
            if sheet_name.lower() in ['осень', 'весна']:
                deleted_count = self.delete_season_entries(sheet, entries_to_delete)
            else:
                deleted_count = self.delete_monthly_entries(sheet, entries_to_delete)
    ...
    def delete_monthly_entries(self, sheet, entries_to_delete):
        ...
        for row_num in rows_to_delete:
            self.delete_monthly_row(sheet, row_num)
        
        return len(rows_to_delete)
    
    def delete_season_entries(self, sheet, entries_to_delete):
        ...
        rows_to_delete.sort(reverse=True)
        for row_num in rows_to_delete:
            self.delete_season_row(sheet, row_num)
        
        return len(rows_to_delete)
    
    def delete_monthly_row(self, sheet, row_num):
    ...
    def delete_season_row(self, sheet, row_num):
    ...
    def fill_season_sheet(self, sheet_name, data):
        ...
        try:
            ...
            existing_row = self.find_existing_season_entry(sheet, data)
            
            if existing_row:
                # Обновляем существующую запись - добавляем часы
                return self.update_season_entry(sheet, existing_row, data)
            else:
                # Создаем новую запись
                return self.create_new_season_entry(sheet, data)
            
        except Exception as e:
            return f"Ошибка: {str(e)}"
    
    def find_existing_season_entry(self, sheet, data):
    ...
    def update_season_entry(self, sheet, row, data):
        ...
        try:
            ...
            # Суммируем часы
            new_lecture = current_lecture + data['lecture']
            new_practice = current_practice + data['practice']
            new_lab = current_lab + data['lab']
            ...
            return f"обновлена строка {row}: +{data['lecture']}л, +{data['practice']}п, +{data['lab']}лаб"
            
        except Exception as e:
            return f"Ошибка обновления строки {row}: {str(e)}"
    
    def create_new_season_entry(self, sheet, data):
        ...
        try:
            ...
            # Записываем часы
            sheet[f'G{row}'] = data['lecture'] if data['lecture'] != 0 else ''
            sheet[f'H{row}'] = data['practice'] if data['practice'] != 0 else ''
            sheet[f'I{row}'] = data['lab'] if data['lab'] != 0 else ''
            
            # Выравнивание
            for col in ['D', 'E', 'F', 'G', 'H', 'I']:
                sheet[f'{col}{row}'].alignment = Alignment(horizontal='center', vertical='center')
            
            return f"строка {row}: {data['discipline']}, {data['group']}, {data['load_type']}, часы: {data['lecture']}л/{data['practice']}п/{data['lab']}лаб"
