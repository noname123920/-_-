...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                               ...
                               QListWidget, QFileDialog, QAbstractItemView, QSlider)
...
class JournalApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.setGeometry(100, 100, 1400, 900)  # Увеличил ширину для размещения видео
        
        # Центральный виджет
        central_widget = QWidget()
        self.setCentralWidget(central_widget)
        
        # Основной layout с сеткой для размещения видео в углу
        main_layout = QGridLayout(central_widget)
        main_layout.setSpacing(10)
        main_layout.setContentsMargins(10, 10, 10, 10)
        
        # Информация о файле - строка 0, колонки 0-1
        self.file_info_label = QLabel("Файл журнала: не загружен")
        self.file_info_label.setStyleSheet("color: red; font-weight: bold;")
        main_layout.addWidget(self.file_info_label, 0, 0, 1, 2)
        
        # Кнопка выбора файла - строка 1, колонки 0-1
        ...
        main_layout.addWidget(self.select_file_btn, 1, 0, 1, 2)
        
        # Период - строка 2, колонки 0-1
        period_group = self.create_period_group()
        main_layout.addWidget(period_group, 2, 0, 1, 2)
        
        # Даты - строка 3, колонки 0-1
        dates_group = self.create_dates_group()
        main_layout.addWidget(dates_group, 3, 0, 1, 2)
        
        # Поля ввода - строка 4, колонки 0-1
        input_group = self.create_input_group()
        main_layout.addWidget(input_group, 4, 0, 1, 2)
        
        # Кнопка добавления - строка 5, колонки 0-1
        ...
        """)
        main_layout.addWidget(self.add_btn, 5, 0, 1, 2)
        
        # Видео плеер - строка 0, колонка 2 (правый верхний угол)
        video_group = self.create_video_group()
        main_layout.addWidget(video_group, 0, 2, 6, 1)  # Занимает строки 0-5, колонку 2
        
        # Просмотр данных - строка 6, колонки 0-2 (на всю ширину)
        view_group = self.create_view_group()
        main_layout.addWidget(view_group, 6, 0, 1, 3)
        
        # Обновляем состояние кнопок
        self.update_ui_state()
    
    def create_video_group(self):
        """Создает группу с видео плеером"""
        group = QGroupBox("Видео")
        group.setMaximumSize(300, 350)  # Ограничиваем размер группы
        layout = QVBoxLayout(group)
        layout.setContentsMargins(5, 5, 5, 5)  # Уменьшаем отступы
        
        # Видео виджет
        self.video_widget = QVideoWidget()
        self.video_widget.setFixedSize(250, 250)  # Фиксированный размер 250x250
        ...
        # Аудио выход и медиа плеер
        self.audio_output = QAudioOutput()
        self.audio_output.setVolume(0.5)  # Устанавливаем громкость 50%
        
        self.media_player = QMediaPlayer()
        self.media_player.setVideoOutput(self.video_widget)
        self.media_player.setAudioOutput(self.audio_output)  # Подключаем аудио выход
        ...
        self.volume_slider.setValue(50)  # Начальное значение 50%
        ...
        self.video_status_label = QLabel("Видео не загружено")
        self.video_status_label.setStyleSheet("color: gray; font-size: 10px;")
    ...
    def load_video(self):
        ...
        try:
            video_path = resource_path("кама.mp4")
            
            # Проверяем существование файла
            if os.path.exists(video_path):
                self.media_player.setSource(QUrl.fromLocalFile(video_path))
                self.video_status_label.setText("Видео загружено: кама.mp4")
                self.video_status_label.setStyleSheet("color: green; font-size: 10px;")
                print(f"Видео загружено: {video_path}")
                
                # Подключаем обработчики событий для отслеживания состояния
                self.media_player.errorOccurred.connect(self.on_player_error)
                self.media_player.mediaStatusChanged.connect(self.on_media_status_changed)
                self.media_player.playbackStateChanged.connect(self.on_playback_state_changed)
                
            else:
                self.video_status_label.setText("Файл кама.mp4 не найден")
                self.video_status_label.setStyleSheet("color: red; font-size: 10px;")
                ...
                alternative_paths = [
                    ...
                    "./кама.mp4", 
                    "../кама.mp4",
                    os.path.join(os.path.dirname(__file__), "кама.mp4")
                ]
                
                for path in alternative_paths:
                    if os.path.exists(path):
                        self.media_player.setSource(QUrl.fromLocalFile(path))
                        self.video_status_label.setText(f"Видео загружено: {os.path.basename(path)}")
                        self.video_status_label.setStyleSheet("color: green; font-size: 10px;")
                        print(f"Видео загружено: {path}")
            ...
            error_msg = f"Ошибка загрузки видео: {e}"
            self.video_status_label.setText(error_msg)
            self.video_status_label.setStyleSheet("color: red; font-size: 10px;")
            print(error_msg)
    
    def on_player_error(self, error, error_string):
        """Обработчик ошибок плеера"""
        print(f"Ошибка медиаплеера: {error_string}")
        self.video_status_label.setText(f"Ошибка: {error_string}")
        self.video_status_label.setStyleSheet("color: red; font-size: 10px;")
    
    def on_media_status_changed(self, status):
        """Обработчик изменения статуса медиа"""
        status_text = {
            QMediaPlayer.MediaStatus.NoMedia: "Нет медиа",
            QMediaPlayer.MediaStatus.LoadingMedia: "Загрузка...",
            QMediaPlayer.MediaStatus.LoadedMedia: "Загружено",
            QMediaPlayer.MediaStatus.StalledMedia: "Буферизация...",
            QMediaPlayer.MediaStatus.BufferingMedia: "Буферизация...",
            QMediaPlayer.MediaStatus.BufferedMedia: "Буферизировано",
            QMediaPlayer.MediaStatus.EndOfMedia: "Конец видео",
            QMediaPlayer.MediaStatus.InvalidMedia: "Неверный формат"
        }
        print(f"Статус медиа: {status_text.get(status, 'Неизвестно')}")
    
    def on_playback_state_changed(self, state):
        """Обработчик изменения состояния воспроизведения"""
        state_text = {
            QMediaPlayer.PlaybackState.StoppedState: "Остановлено",
            QMediaPlayer.PlaybackState.PlayingState: "Воспроизведение", 
            QMediaPlayer.PlaybackState.PausedState: "Пауза"
        }
        print(f"Состояние воспроизведения: {state_text.get(state, 'Неизвестно')}")
    
    def play_video(self):
        """Воспроизводит видео"""
        try:
            if self.media_player.source().isEmpty():
                self.load_video()  # Попробуем перезагрузить видео
                
            self.media_player.play()
            self.video_status_label.setText("Воспроизведение...")
            self.video_status_label.setStyleSheet("color: blue; font-size: 10px;")
    ...
    def pause_video(self):
        ...
        try:
            ...
            self.video_status_label.setText("Пауза")
            self.video_status_label.setStyleSheet("color: orange; font-size: 10px;")
        except Exception as e:
            print(f"Ошибка паузы видео: {e}")
    
    def stop_video(self):
        """Останавливает видео"""
        try:
            self.media_player.stop()
            self.video_status_label.setText("Остановлено")
            self.video_status_label.setStyleSheet("color: gray; font-size: 10px;")
        except Exception as e:
    ...
    def set_volume(self, value):
        ...
        try:
            ...
            print(f"Громкость установлена: {value}%")
        except Exception as e:
            print(f"Ошибка установки громкости: {e}")

    # ... остальные методы без изменений (update_ui_state, create_period_group, и т.д.)
    ...
    # [Все остальные методы из предыдущего кода остаются без изменений]
